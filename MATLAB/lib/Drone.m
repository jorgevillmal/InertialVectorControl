classdef Drone < handle
    ...
    methods
        ...
        function obj = PositionCtrl(obj, pos_des, vel_des, acc_des)
           ...
           % 
           %  % Agregar el primer y último valor para mantener el mismo tamaño
           %  obj.v_des = [obj.v_des(:, 1), obj.v_des, obj.v_des(:, end)];
           %  disp('v_des')
           %  disp(obj.v_des)
           % 
           % %%%%%%%%%%%%%%%%%%% Acceleration %%%%%%%%%%%%%%%%%%%%%%
           ...
           % obj.v_des = (obj.x_des - obj.prev_x_d) / obj.dt;   % Primera derivada (velocidad)
           % disp('v_des')
           % disp(obj.v_des)
           % 
           % obj.ddx_d = (obj.v_des - obj.prev_dx_d) / obj.dt;  % Segunda derivada (aceleración)
           % disp('ddx_d')
           % disp(obj.ddx_d)
           % 
           % 
           % 
           % obj.prev_x_d = obj.x_des;
           % obj.prev_dx_d = obj.v_des;


           % obj.v_des = (obj.next_x_d - obj.x_des) / obj.dt;
           % disp('v_des')
           % disp(obj.v_des)
           % 
           % obj.ddx_d = (obj.next_dx_d - obj.v_des) / obj.dt;
           % disp('ddx_d')
           % disp(obj.ddx_d)
           % 
           % obj.x_des = obj.next_x_d;
           % obj.v_des = obj.next_dx_d;


           % obj.v_des = (obj.next_x_d - obj.prev_x_d) / (2 * obj.dt);   % Primera derivada (velocidad)
           % disp('v_des')
           % disp(obj.v_des)
           % 
           % obj.ddx_d = (obj.next_dx_d - obj.prev_dx_d) / (2 * obj.dt);  % Segunda derivada (aceleración)
           % disp('ddx_d')
           % disp(obj.ddx_d)
           % 
           % obj.prev_x_d = obj.x_des;
           ...
           % obj.x_des = pos_des;
           % %%%%%%%%%%%%%%%%%%   Velocity   %%%%%%%%%%%%%%%%%%%%%%
           % obj.v_des = [centralDifference(obj.x_des, obj.dt)];
           % disp('v_des')
           % disp(obj.v_des)
           % %%%%%%%%%%%%%%%%%% Acceleration %%%%%%%%%%%%%%%%%%%%%%
           % obj.dotV_des = [centralDifference(obj.v_des, obj.dt)];
           ...
           % obj.x_des = pos_des;
           % 
           % % Asumiendo que obj.x_des ya ha sido asignado y tiene dimensiones 3xN
           % N = size(obj.x_des, 2);
           % 
           ...
           % 
           % %%%%%%%%%%%%%%%%%%%   Velocity   %%%%%%%%%%%%%%%%%%%%%%
           % % Calculating for each row of obj.x_des separately
           % obj.v_des(1, :) = centralDifference(obj.x_des(1, :), obj.dt);
           % obj.v_des(2, :) = centralDifference(obj.x_des(2, :), obj.dt);
           % obj.v_des(3, :) = centralDifference(obj.x_des(3, :), obj.dt);
           % disp('v_des')
           % disp(obj.v_des)
           % 
           % %%%%%%%%%%%%%%%%%% Acceleration %%%%%%%%%%%%%%%%%%%%%%
           % % Calculating for each row of obj.v_des separately
           % obj.dotV_des(1, :) = centralDifference(obj.v_des(1, :), obj.dt);
           % obj.dotV_des(2, :) = centralDifference(obj.v_des(2, :), obj.dt);
           % obj.dotV_des(3, :) = centralDifference(obj.v_des(3, :), obj.dt);
           % obj.ddx_d = obj.dotV_des;
           % disp('ddx_d')
           % disp(obj.ddx_d)
